...
def do_snail_homework():
    ...
    magnitudes = []
    number_permutations = list(permutations(number_list, 2))
    print(f"max number_permutations: {len(number_permutations)}")
    start_time = time.time()
    for i in range(len(number_permutations)):
        magnitudes.append(perform_snail_operation(number_permutations[i]))
        elapsed_time = (time.time() - start_time)           
        average_time = elapsed_time / (i + 1)
        remaining_time = average_time * (len(number_permutations) - (i + 1))
        print(f"permutation {i} complete.  --- {remaining_time} seconds remaining ---" )

    print(f"max magnitude: {max(magnitudes)}")

def perform_snail_operation(number_list: List[str]):
    ...
    magnitude = calculate_magnitude(eval(snail_number))
